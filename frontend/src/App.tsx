...
const PRIVATE_KEY_WITH_BALANCE = "";
...
function App() {
    ...
    useEffect(() => {
        ...
        const wallet = PRIVATE_KEY_WITH_BALANCE as any === ""
            ? Wallet.generate({provider})
            : Wallet.fromPrivateKey(PRIVATE_KEY_WITH_BALANCE, provider);
        CounterContractAbi__factory
            .connect(COUNTER_ID, wallet)
            .functions
            .count()
            .simulate()
    ...
    return (
        <div className="App">
            <h3>Counter: {counter == null ? "loading" : counter?.toFixed(0)}</h3>
            {/*{*/}
            {/*    isConnected ? (*/}
